...
Queue.prototype.enqueue = function (name, params, options, checkExists, callback) {
    ...
    if (args.length > 0) checkExists = args.shift(); else checkExists = null;

    // if options has check param , then it is for checkExists
    if (options.hasOwnProperty('check')) {
        checkExists = options;
        delete checkExists.check;
        options = {};
    }
    ...
    job.enqueue(checkExists, callback);
};
